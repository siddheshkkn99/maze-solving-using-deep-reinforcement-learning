...
class MazeGen:
    ...
    def __init__(self, nrows, ncols):
        # 0 - blocked, 1 - free, initialising maze with all blocked cells
        self.maze = np.array([[0.0 for x in range(nrows)] for y in range(ncols)])
        # setting rat(agent) and cheese(target) cells
        self.rat = (0, 0)
        self.cheese = (nrows - 1, ncols - 1)
    ...
    # function to validate a move by checking if the rat movement is restriceted by the sides of the maze or not
    def validAction(self):
        # all possible moves: left, right, up and down
        validActions = ['l', 'r', 'u', 'd']
        nrow, ncol = self.maze.shape
        row, col = self.rat
        if   row == 0.0     : validActions.remove('u')
        elif row == nrow - 1: validActions.remove('d')
        if   col == 0.0     : validActions.remove('l')
